...
from datasets.dataset import SentimentSentimentDataset
from scripts.logging import create_logger
from scripts.utils import prepare_batch_for_bert_model
from models import BERT_Model

def parse_args():
    parser = argparse.ArgumentParser(description='Train IMDB Sentiment Analysis Training')
...
def main():
    ...
    data = SentimentSentimentDataset(
        root=opts.root,
    ...
    train_loader = DataLoader(trainset, batch_size=opts.batch_size, shuffle=True, pin_memory=True, num_workers= int(multiprocessing.cpu_count()/2), drop_last=True)
    val_loader = DataLoader(valset, batch_size=opts.batch_size, shuffle=True, pin_memory=True, num_workers= int(multiprocessing.cpu_count()/2), drop_last=True)
